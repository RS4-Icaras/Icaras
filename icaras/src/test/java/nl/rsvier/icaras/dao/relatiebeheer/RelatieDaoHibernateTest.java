...
public class RelatieDaoHibernateTest {
	...
	public void setUp() throws InvalidBusinessKeyException{
		...
		testRelatie1.setGeboortedatum(new GregorianCalendar(1988, GregorianCalendar.FEBRUARY, 13));
		testRelatie2 = new Organisatie("Organisatienaam");//kan exception gooien
	...
	public void testSaveEnGet() {
		...
		assertTrue("attributen vanuit database zijn gelijk aan die van de relatie voor save", testRelatie1.equals(testRelatie2));
	}
	@Test
	@Transactional
	public void testGetAll(){
		...
		{for (Relatie n : nfaLijst){ if (n.getId()==testRelatie1.getId()) vergelijkRelatie1 = n;}};
		{for (Relatie n : nfaLijst){ if (n.getId()==testRelatie2.getId()) vergelijkRelatie2 = n;}};
		assertTrue("eerste opgeslagen relatie en relatie uit de opgehaalde lijst met dezelfde id zijn gelijk", testRelatie1.equals(vergelijkRelatie1));
		assertTrue("tweede opgeslagen relatie en relatie uit de opgehaalde lijst met dezelfde id zijn gelijk", testRelatie2.equals(vergelijkRelatie2));
	}
	...
	public void testDelete(){
		...
		relatieDaoHibernate.getHibernateTemplate().evict(testRelatie1);
		testRelatie2 = relatieDaoHibernate.getById(testRelatie1.getId());
		assertNull("verwijderde nfa opvragen uit database geeft null", testRelatie2);
	}
	...
	public void testUpdate(){
		...
		relatieDaoHibernate.getHibernateTemplate().flush();
		relatieDaoHibernate.getHibernateTemplate().evict(testRelatie1);
		Relatie vergelijkNfa = relatieDaoHibernate.getById(testRelatie1.getId());
		assertTrue("attributen geupdate adres is gelijk aan attributen ingeladen adres met dezelfde id", testRelatie1.equals(vergelijkNfa));
	}
}
