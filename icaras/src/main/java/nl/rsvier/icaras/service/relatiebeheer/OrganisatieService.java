...
public class OrganisatieService implements IOrganisatieService {
	...
	public void delete(Organisatie organisatie) {
		...
		if(organisatie.heeftRol(Bedrijf.class)){
			...
			//deze hashSet wordt aangemaakt om ConcurrentModicificationError in for loop
			//te omzeilen
			HashSet<Arbeidsovereenkomst> arbeidsovereenkomsten = new HashSet<Arbeidsovereenkomst>();
			arbeidsovereenkomsten.addAll(organisatie.getBedrijf().getArbeidsovereenkomsten());
	   ...
	   //na het verwijderen van alle bidirectionele relaties verwijderen we persoon
		//alle niet bidirectionele relaties van Organisatie worden via OrphanRemove verwijderd
